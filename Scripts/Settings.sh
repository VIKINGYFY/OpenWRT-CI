...
if [[ $OWRT_URL == *"lede"* ]] ; then
  #修改默认主题为Design
  sed -i 's/luci-theme-bootstrap/luci-theme-design/g' $(find ./feeds/luci/collections/luci/ -type f -name "Makefile")
  #修改默认时间格式
  sed -i 's/os.date()/os.date("%Y-%m-%d %H:%M:%S %A")/g' $(find ./package/*/autocore/files/ -type f -name "index.htm")
elif [[ $OWRT_URL == *"immortalwrt"* ]] ; then
  #修改默认主题为Argon
  sed -i 's/luci-theme-bootstrap/luci-theme-argon/g' $(find ./feeds/luci/collections/luci/ -type f -name "Makefile")
fi
